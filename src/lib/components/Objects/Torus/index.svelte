<script lang="ts">
  ...
  export let options = {} as Parameters<typeof BABYLON.MeshBuilder.CreateTorus>[1]

  const context = createObjectContext(BABYLON.MeshBuilder.CreateTorus(name, options, root.scene))
