<script lang="ts">
  ...
  export let name: string = 'TorusKnot'
  export let receiveShadows = false
  export let options = {} as Parameters<typeof MeshBuilder.CreateTorusKnot>[1]

  const parentObject = getContext('object') as {
    self: Mesh | AbstractMesh
  }
  export let parent: Node = parentObject?.self
  const context = createObjectContext(MeshBuilder.CreateTorusKnot(name, options, root.scene))
