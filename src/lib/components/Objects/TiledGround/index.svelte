<script lang="ts">
  ...
  export let options: Parameters<typeof MeshBuilder.CreateTiledGround>[1]

  const parentObject = getContext('object') as {
    self: Mesh | AbstractMesh
  }
  export let parent: Node = parentObject?.self
  const context = createObjectContext(MeshBuilder.CreateTiledGround(name, options, root.scene))
