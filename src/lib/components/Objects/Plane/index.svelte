<script lang="ts">
  ...
  export let options = {} as Parameters<typeof BABYLON.MeshBuilder.CreatePlane>[1]
  ...
  const context = createObjectContext(BABYLON.MeshBuilder.CreatePlane(name, options, root.scene))
